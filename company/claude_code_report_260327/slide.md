...
- Claude Codeに実装を書いてもらう→プッシュ前にリンター・テストを実行→デプロイ、の流れを作成してもらった。
...
- プロンプト：
```
Issue #2を実装してください。別ブランチを立てて、最後プルリクエストをお願いします。
```
...
想定どおりに動いてくれないときがある。カスタムスラッシュコマンドで複数のコマンドをまとめておく。
...
コマンド実行例）Issue #44のプルリクエスト#92をレビュー
```
/pr-review 92
```
